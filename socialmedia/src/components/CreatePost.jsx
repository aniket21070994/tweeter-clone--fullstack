import React from 'react'
import { CiImageOn } from "react-icons/ci";
import Avatar from 'react-avatar'
export const CreatePost = () => {
  return (
    <div className='w-[100%] mt-2 '>
       
        <div className=''>  
           <div className='flex border border-b-2 border-gray-300 items-center justify-between '>
               <div className='font-bold text-gray-600 texte-lg cursor-pointer hover:bg-gray-300 w-full text-center px-4 py-3'><h1>For you</h1></div>
               <div className='font-bold text-gray-600 texte-lg cursor-pointer hover:bg-gray-300  w-full text-center px-4 py-3'><h1>Following</h1></div>
          </div>
        </div>
            <div className='m-4 flex'>
                <div className=' rounded-full border border-gray-200'>
                     <Avatar src='data:image/png;base64,' size="40" round={true}  />
                 </div>
                 <input className='w-full ml-1 outline-none border border-none text-xl' type="text" placeholder='What is happening'/>
           </div>
           <div className='flex justify-between m-2 items-center'>
            <div >
                <CiImageOn  className ="cursor-pointer" size='24px'/>
            </div>
            <button className='bg-[#109BF0] text-lg text-white  px-4 py-1 border-none rounded-full'>Post</button>
           </div>
       
    </div>
  )
}
