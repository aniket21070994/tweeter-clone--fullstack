import React, { useState } from 'react'

const Login = () => {
  const [isLogin,setLogin]=useState(true)
  const LoginSignupHandeler=()=>{
    setLogin(!isLogin)
  }
  return (
    <div className='w-screen h-screen flex items-center justify-center'>
       <div className='flex items-center justify-evenly w-[80%]'>
         <div>
             <img className='ml-5 w-[190px]' src="data:image/png;base64," alt="" />
         </div>
         <div>
          <div className='my-5'>
            <h1 className='font-bold text-6xl'>Happening now</h1>
          </div>
          <h1 className='mt-4 mb-2 text-2xl font-bold'>{isLogin?"Login":"Signup"}</h1>
            <form className='flex flex-col w-[50%] font-semibold' action="">
             
             { !isLogin &&(
              <>
            
                <input type="text" placeholder='Name'className='outline-blue-500 border border-gray-800 rounded-full px-3 py-1 my-1' />
                <input type="text" placeholder='Username' className='outline-blue-500 border border-gray-800 rounded-full px-3 py-1 my-1' />
                </>
             )
            }
              <input type="text" placeholder='Email' className='outline-blue-500 border border-gray-800 rounded-full px-3 py-1 my-1'/>
              <input type="text" placeholder='Password' className='outline-blue-500 border border-gray-800 rounded-full px-3 py-1 my-1'/>
              
               <button className='bg-[#1D9BF0] border-non py-2 my-4 rounded-full text-lg text-white'>{isLogin?"Login":"Signup"}</button>
               <h1 className='w-full'>{isLogin?"Do not have an account?":"Alredy have an account?" }<span onClick={LoginSignupHandeler} className='font-bold cursor-pointer text-blue-500 '>{isLogin?"Register":"Login"}</span></h1>
            </form>
         </div>
       </div>
    </div>
  )
}

export default Login