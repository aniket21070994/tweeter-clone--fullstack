import React from 'react'
import Avatar from 'react-avatar';
import { CiSearch } from "react-icons/ci";
import { getOtherUser } from '../redux/userSlice';
import { useSelector } from 'react-redux';
import { Link } from 'react-router-dom';

function RightSidebar() {
  const {otherUser}=useSelector(store=>store.user)
  return (
    <div className='w-[20%] '>
      <div className='flex items-center p-2 bg-gray-300 rounded-full outline-none w-full'>
        <CiSearch size='20px' />
        <input type="text" className=' bg-transparent outline-none px-2 ' placeholder='Search' />
      </div>



      <div className='py-2 m-1 border border-gray-300 bg-gray-200 w-full rounded-2xl my-4'>
        <div className='font-bold px-2 text-lg items-center '>
          <h1>who to Follow</h1>
          <div>

           {
            otherUser?.map(user=>{
              return(
              <div key={user?._id} className=' mt-4 flex items-center w-full rounded-full '>

              <div>
                <Avatar src='data:image/png;base64,' size="40" round={true} />
              </div>
              <div className='items-center'>
                <h1 className='font-semibold '>{user?.name}</h1>
                <p className='text-xs'>{`@${user.username}`}</p>
              </div>
              
              <div className='ml-2 position absolute right-[12%]'>
              <Link to={`/profile/${user._id}`}>
              <button className='w-full  left-[40%] bg-black px-2 py-1 rounded-full font-semibold texte-sm text-white'>Profile</button>
              </Link>
               </div>
            </div>
              )  
          })
           }

          </div>
        </div>
      </div>

    </div>
  )
}

export default RightSidebar