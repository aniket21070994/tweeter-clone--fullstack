import React from 'react'
import Avatar from 'react-avatar';
import { CiSearch } from "react-icons/ci";
const Users=()=>{
  return( 
  <div className=' mt-4 flex items-center w-full rounded-full '>
        
  <div> 
    <Avatar src='data:image/png;base64,' size="40" round={true}  />
  </div>
  <div className='items-center'>
  <h1 className='font-semibold '>Aniket Mankar</h1>
  <p className='text-xs'>@aniketmankar1904</p>
  </div>
  <div className='ml-2'><button className='w-full bg-black px-3 py-1 rounded-full font-semibold texte-sm text-white'>Profile</button></div>
</div>
)
}
function RightSidebar() {
  return (
    <div className='w-[20%] '>
      <div className='flex items-center p-2 bg-gray-300 rounded-full outline-none w-full'>
        <CiSearch size='20px'/>
        <input type="text" className=' bg-transparent outline-none px-2 ' placeholder='Search' />
      </div>
      
      
      
      <div className='py-2 m-1 border border-gray-300 bg-gray-200 w-full rounded-2xl my-4'>
      <div className='font-bold px-2 text-lg items-center '>
        <h1>who to Follow</h1>
      <div>

        <Users/>
        <Users/>
        <Users/>
        <Users/>
     
      </div>
      </div>
      </div>
    
    </div>
  )
}

export default RightSidebar